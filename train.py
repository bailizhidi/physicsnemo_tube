...
class MGNTrainer:
    ...
    def forward(self, graph, mesh_edge_features, world_edge_features):
        with autocast("cuda", enabled=self.amp):
            ...
            # 分别计算各项物理误差 (方便后期调整权重)
            # pred 和 graph.y 均为 6维: [Vel_x, Vel_y, Vel_z, Stress, Strain, Peeq]
            loss_vel = self.criterion(pred[:, 0:3], graph.y[:, 0:3])
            loss_stress = self.criterion(pred[:, 3:4], graph.y[:, 3:4])
            loss_strain = self.criterion(pred[:, 4:5], graph.y[:, 4:5])
            loss_peeq = self.criterion(pred[:, 5:6], graph.y[:, 5:6])

            # 将各项误差加权合并 (过拟合阶段均设为 1.0)
            total_loss = loss_vel + loss_stress + loss_strain + loss_peeq
            
            # (可选) 你还可以加个 print 来观察分别是哪一项难收敛
            # print(f"Vel: {loss_vel.item():.4f}, Stress: {loss_stress.item():.4f}")
